...
{#await useGltf(`${CLOUDINARY_BASE_URL}/assets/ghost.glb`, { loader: dracoGltfLoader }) then ghost}
	<T is={ghost.scene} position={[0, y, 0]} scale={0.4} />
{/await}

{#await useGltf(`${CLOUDINARY_BASE_URL}/assets/garden.glb`, { loader: dracoGltfLoader }) then garden}
	<T is={garden.scene} rotation.y={rotation} />
{/await}

{#await useGltf(`${CLOUDINARY_BASE_URL}/public/nissan2.glb`, { loader: dracoGltfLoader }) then nissan}
