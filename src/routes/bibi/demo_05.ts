...
export async function initScene(canvas: HTMLCanvasElement) {
  ...
  const gltf = await loader.loadAsync(getCloudinaryAssetUrl('Barrel Model1.glb')) //('/public/mario.glb')
  ...
  const texture = new THREE.TextureLoader().load(getCloudinaryAssetUrl('mario.png'))
